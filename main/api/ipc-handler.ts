...
export function registerHandlers(webContents: WebContents): void {
	...
	ipcMain.handle("search-users", async (event: IpcMainInvokeEvent, args: string): Promise<string[]> => {
		
		if (bot === undefined) throw getUninitializedBotError("search-users");
		else return (await bot.directory().query(args)).map<string>((user: IHCBotUser): string => user.username());
